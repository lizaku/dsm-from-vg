...
def read_entities():
    entities = {}
    with open("data-liza/entities.txt") as f:
...
def make_predicates(use_attributes, use_relations, use_hypernyms, use_situations):
    predicate_count = 0
    i_to_predicates = {}
    predicates_to_i = {}
    ...
    predicate_count = record_predicates("data-liza/synset_freqs.txt", predicate_count)
    if use_attributes == 'True':
        predicate_count = record_predicates("data-liza/attribute_freqs.txt", predicate_count)
    if use_relations == 'True':
        predicate_count = record_predicates("data-liza/relation_freqs.txt", predicate_count)
...
path = 'spaces/' + sys.argv[1]
use_attributes = sys.argv[2]
use_relations = sys.argv[3]
use_hypernyms = sys.argv[4]
use_situations = sys.argv[5]
...
i_to_predicates, predicates_to_i = make_predicates(use_attributes=use_attributes,
...
with open("data-liza/ideallanguage.txt") as f:
    lines = f.read().splitlines()
...
write_dictionary(entity_matrix, path + "/entity_matrix.dm")

predicate_matrix = aggregation(entity_matrix,inverse_entity_matrix, predicates_to_i)
write_numpy_matrix(predicate_matrix, i_to_predicates, path + "/predicate_matrix.dm")

prob_matrix = prob_interpretation(predicate_matrix)
write_numpy_matrix(prob_matrix, i_to_predicates, path + "/probabilistic_matrix.dm")
